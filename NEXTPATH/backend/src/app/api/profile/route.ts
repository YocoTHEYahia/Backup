...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json({ profile: related });
  } catch (error) {
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // Only fields that target `profiles` itself are forwarded to the
    // repository. Related-table writes are handled by dedicated routes.
